...
namespace OrderManager
{
    ...
    public class Order:IComparable
    {
        ...
        public void AddItem(string name, int uPrice, int num)
        {
            //传入 货品名称，单价，数量
            try
            {
                OrderDetail obj = new OrderDetail(this.turnId, name, uPrice, num);
                foreach (OrderDetail item in this.items)
                {
                    if (item.Equals(obj))
                    {
        ...
        public void AddItem(OrderDetail item, string name, int uPrice, int num)
        {
            item.Id = this.itemId;
            this.ModifyItem(this.itemId, name, uPrice, num);
            this.itemId += 1;
        }
